...
export default async function handler(req, res) {
    ...
    authenticate(req, res, async () => {
        ...
        try {
            ...
            const [customer] = await connection.query("SELECT * FROM Customers WHERE customer_id = ? AND account_id = ?", [customer_id, account_id]);
            ...
            const [existingProducts] = await connection.query("SELECT * FROM Products WHERE product_id IN (?) AND account_id = ?", [productIds, account_id]);
